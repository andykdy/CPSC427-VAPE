...
void EntityGrid::addToGrid(const Player &player) {
    vec2 box = player.get_bounding_box();
...
void EntityGrid::addToGrid(const Vamp &vamp) {
    vec2 box = vamp.get_bounding_box();
    vec2 pos = vamp.get_position();
    ...
    addBoxToGrid(tl, br, EType::vamp);
}

void EntityGrid::addToGrid(const Pickup &pickup) {
    vec2 box = pickup.get_bounding_box();
    vec2 pos = pickup.get_position();
...
void EntityGrid::addToGrid(const Enemy &enemy) {
    vec2 box = enemy.get_bounding_box();
    vec2 pos = enemy.get_position();
...
void EntityGrid::addToGrid(const Boss &boss) {
    vec2 box = boss.get_bounding_box();
    vec2 pos = boss.get_position();
...
void EntityGrid::addToGrid(const Projectile &projectile) {
    vec2 box = projectile.get_bounding_box();
    if (box.x == -1 && box.y == -1){
        return;
    }
    vec2 pos = projectile.get_position();
    ...
    EType type = projectile_friendly;
    if (projectile.isHostile()) {
        type = projectile_hostile;
    }


    addBoxToGrid(tl, br, type);
}
...
void EntityGridSquare::draw(const mat3 &projection, vec2 pos, EType type) {
    ...
    switch(type) {
        case EType::player: {
            ...
            color[1] = 1.f;
            break;
        } case EType::projectile_hostile: {
            color[0] = 0.7f;
            color[1] = 0.1f;
            color[2] = 0.1f;
            break;
        } case EType::projectile_friendly: {
            color[0] = 0.7f;
            color[1] = 0.4f;
            color[2] = 0.1f;
            break;
